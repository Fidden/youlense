...
bool ui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
        window->DrawList->AddRectFilled(pos + ImVec2(2, 2), pos + ImVec2(10, 10), g_menu.get_accent_color());
    else {
        if (hovered)
...
bool ui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt)
{
    ...
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), g_menu.get_accent_color());
...
bool ui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->PushClipRect(bb.Min, bb.Max, true);
    window->DrawList->AddText(bb.Min + ImVec2(8, size.y / 2 - label_size.y / 2 + 2), selected ? g_menu.get_accent_color() : ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), label);
    window->DrawList->PopClipRect();
