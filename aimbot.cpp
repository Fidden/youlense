...
void AimPlayer::UpdateAnimations(LagRecord* record) {
	...
	bool fake = !bot && g_config.b["aimbot_resolver"];
...
void AimPlayer::OnNetUpdate(Player* player) {
	bool reset = (!g_config.b["aimbot_enable"] || player->m_lifeState() == LIFE_DEAD || !player->enemy(g_cl.m_local));
...
void AimPlayer::SetupHitboxes(LagRecord* record, bool lc) {
	...
	// prefer, always.
	if (g_config.m["aimbot_prefer_baim"][0])
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });

	// prefer, lethal.
	if (g_config.m["aimbot_prefer_baim"][1])
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::LETHAL });

	// prefer, lethal x2.
	if (g_config.m["aimbot_prefer_baim"][2])
	...
	// only, always.
	if (g_config.m["aimbot_only_baim"][0]) {
		only = true;
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });
	}
	...
	auto hitbox =  /*history ? g_config.m["aimbot_target_history_hitbox"] : */ g_config.m["aimbot_target_hitbox"];
	bool ignore_limbs = record->m_velocity.length_2d() > 71.f && g_config.b["aimbot_ignore_limbs"];
...
void Aimbot::UpdatePingBind( ) {
	in_latency = g_config.auto_check("fakeping_key") && g_config.b[XOR("misc_fakeping")];
}

void Aimbot::UpdateBaimBind() {
	in_baim = g_config.auto_check("rage_forcebaim_key") && g_config.b[XOR("rage_forcebaim")];
}
...
void Aimbot::think() {
	...
	if (g_cl.m_weapon_type == WEAPONTYPE_KNIFE && g_cl.m_weapon_id != ZEUS) {
		...
		if (g_config.b["aimbot_knife_bot"])
			knife();
...
void Aimbot::find() {
	...
	if (g_cl.m_weapon_id == ZEUS && !g_config.b["aimbot_zeus_bot"])
		return;
	...
	if (best.player && best.record) {
		...
		if (can_scope) {
			// always.
			if (g_config.i["aimbot_scope"] == 1) {
				g_cl.m_cmd->m_buttons |= IN_ATTACK2;
				return;
			}

			// hitchance fail.
			else if (g_config.i["aimbot_scope"] == 2 && on && !hit) {
				g_cl.m_cmd->m_buttons |= IN_ATTACK2;
				return;
			}
		}
...
bool AimPlayer::SetupHitboxPoints(LagRecord* record, BoneArray* bones, int index, std::vector< vec3_t >& points) {
	...
	// get hitbox scales.
	float scale = g_config.i["aimbot_multipoint_scale"] / 100.f;

	// big inair fix.
	if (!(record->m_pred_flags & FL_ONGROUND))
		scale = 0.7f;

	float bscale = g_config.i["aimbot_body_scale"] / 100.f;
	...
	if (bbox->m_radius <= 0.f) {
		...
		if (index == HITBOX_R_FOOT || index == HITBOX_L_FOOT) {
			...
			if (g_config.m["aimbot_target_multipoint"][3]) {
	...
	// these hitboxes are capsules.
		...
		if (index == HITBOX_HEAD) {
			...
			if (g_config.m["aimbot_target_multipoint"][0]) {
		...
		// body has 5 points.
			...
			// back.
			if (g_config.m["aimbot_target_multipoint"][2])
		...
		// other stomach/chest hitboxes have 2 points.
			...
			if (g_config.m["aimbot_target_multipoint"][1])
				points.push_back({ center.x, bbox->m_maxs.y - r, center.z });
		}
...
bool AimPlayer::GetBestAimPosition(vec3_t& aim, float& damage, LagRecord* record) {
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		dmg = g_config.i["aimbot_minimal_dmg"];
		if (g_config.b["aimbot_minimal_dmg_scale"])
			dmg = std::ceil((dmg / 100.f) * hp);

		pendmg = g_config.i["aimbot_minimal_dmg_pen"];
		if (g_config.b["aimbot_minimal_dmg_pen_scale"])
...
bool Aimbot::SelectTarget(LagRecord* record, const vec3_t& aim, float damage) {
	...
	switch (g_config.i["aimbot_target_selection"]) {
...
void Aimbot::apply() {
	...
	if (attack || attack2) {
		...
		*g_cl.m_packet = false;
		...
		// nospread.
		if (g_config.b["aimbot_remove_spread"] && !g_config.b["anti_untrasted"])
			NoSpread();

		// norecoil.
		if (g_config.b["aimbot_remove_recoil"])
