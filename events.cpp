...
void events::round_start(IGameEvent* evt) {
	...
	for (auto hk : g_lua_hook.getHooks("on_round_start"))
		hk.func();
...
void events::round_end(IGameEvent* evt) {
	...
	for (auto hk : g_lua_hook.getHooks("on_round_end"))
		hk.func();
}
...
void events::player_hurt(IGameEvent* evt) {
	...
	if (evt && g_config.m["misc_notifications"][2]) {
	...
	for (auto hk : g_lua_hook.getHooks("on_player_hurt"))
		hk.func();
}
...
void events::bullet_impact(IGameEvent* evt) {
	if (g_cl.m_processing) 
	{
		...
		for (auto hk : g_lua_hook.getHooks("on_bullet_impact"))
			hk.func();
	}
}
...
void events::item_purchase(IGameEvent* evt) {
	...
	for (auto hk : g_lua_hook.getHooks("on_item_purchase"))
		hk.func();

	if (!g_config.m["misc_notifications"][1])
		return;
...
void events::player_death(IGameEvent* evt) {
	...
	for (auto hk : g_lua_hook.getHooks("on_player_death"))
		hk.func();
}
...
void events::player_given_c4(IGameEvent* evt) {
	...
	if (!g_config.m["misc_notifications"][3])
		return;
