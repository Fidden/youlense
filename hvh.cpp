...
void HVH::AntiAimPitch() {
	bool safe = g_config.b["anti_untrasted"];
...
void HVH::GetAntiAimDirection() {
	// edge aa.
	if (g_config.b["antiaim_edge"] && g_cl.m_local->m_vecVelocity().length() < 320.f) {
	...
	// lock while standing..
	bool lock = g_config.b["antiaim_lockdir"];
...
void HVH::DoRealAntiAim() {
	...
	if (m_yaw > 0) {
		...
		if (stand && !g_cl.m_lag && g_csgo.m_globals->m_curtime >= (g_cl.m_body_pred - g_cl.m_anim_frame) && g_csgo.m_globals->m_curtime < g_cl.m_body_pred) {
			// z mode.
			if (g_config.i["antiaim_body_yaw"] == 4)
		...
		if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air) ) {
			...
			switch (g_config.i["antiaim_body_yaw"]) {
			// left.
			case 1:
		...
		// run normal aa code.
			...
			case 4:
				...
				if (g_csgo.m_globals->m_curtime >= m_next_random_update) {
					...
					m_next_random_update = g_csgo.m_globals->m_curtime + 5.f;
				}
...
void HVH::DoFakeAntiAim() {
	...
	*g_cl.m_packet = true;
	...
	switch (g_config.i["antiaim_fake_yaw"]) {
	...
	case 2:
		...
		g_cl.m_cmd->m_view_angles.y += g_config.i["antiaim_relative_yaw"];
		break;

		// relative jitter.
	case 3: {
		// get fake jitter range from menu.
		float range = g_config.i["antiaim_yaw_add"] / 2.f;
...
void HVH::AntiAim() {
	...
	// load settings.
	m_pitch = g_config.i["antiaim_pich"];
	m_yaw = g_config.i["antiaim_yaw"];
	m_jitter_range = g_config.i["antiaim_yaw_add"];
	m_rot_range = g_config.i["antiaim_rotate_range"];
	m_rot_speed = g_config.i["antiaim_rotate_speed"];
	m_dir = g_config.i["antiaim_yaw_direction"];
	m_dir_custom = g_config.i["antiaim_yaw_direction_custom"];
	m_base_angle = g_config.i["antiaim_yaw_base"];
	m_auto_time = g_config.i["antiaim_yaw_direction_custom"];
	...
	else if (g_config.i["antiaim_fake_yaw"] > 0)
		m_direction = g_cl.m_cmd->m_view_angles.y;

	if (g_config.i["antiaim_fake_yaw"] > 0) {
...
void HVH::SendPacket() {
	...
	if (!g_config.b["fakelag_enable"])
		return;

	// fake-lag enabled.
	if (!g_csgo.m_gamerules->m_bFreezePeriod() && !(g_cl.m_flags & FL_FROZEN)) {
		// limit of lag.
		int limit = std::min((int)g_config.i["fakelag_limit"], g_cl.m_max_lag);
		...
		auto activation = g_config.m["fakelag_trigger"];
		// move.
		if (activation[0] && delta > 0.1f && g_cl.m_speed > 0.1f) {
			active = true;
		}

		// air.
		else if (activation[1] && ((g_cl.m_buttons & IN_JUMP) || !(g_cl.m_flags & FL_ONGROUND))) {
			active = true;
		}

		// crouch.
		else if (activation[2] && g_cl.m_local->m_bDucking()) {
			active = true;
		}
		

		if (active) {
			int mode = g_config.i["fakelag_mode"];
	...
	if (g_config.auto_check("antiaim_fakewalk_key")) {
		*g_cl.m_packet = false;
	}
